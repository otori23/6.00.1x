...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ## YOUR CODE HERE ####    
        aAncestors = []
        current = self.names_to_nodes[a] 
        while current != None:
            aAncestors.append(current)
            current = current.get_parent()

        bAncestors = []
        current = self.names_to_nodes[b]
        while current != None:
            bAncestors.append(current)
            current = current.get_parent()
        
        commonNode = None        
        for aNode in aAncestors:
            if aNode in bAncestors:
                commonNode = aNode
                break
        
        aLen = aAncestors.index(commonNode) - 1
        bLen = bAncestors.index(commonNode) - 1
        
        cousinType = min(aLen, bLen)
        degreesRemoved = abs(aLen - bLen)
