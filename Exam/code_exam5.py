...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        for oneCourse in self.myCourses:
            if oneCourse.courseName == course:
                oneCourse.setGrade(grade)
    ...
    def getPset(self, pset, course="6.00x"):
        ...
        for oneCourse in self.myCourses:
            if oneCourse.courseName == course:
                thePset = oneCourse.getPset(pset)
                break
        else:
            thePset = -1        
        return thePset
